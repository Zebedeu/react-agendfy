  ...
  const getEvents = useCallback(
    (slotTime: string) => {
      ...
      return events.reduce((acc: EventProps[], event: EventProps) => {
          ...
          try {
            ...
            occurrences.forEach((occurrence) => {
              const occurrenceStart = setMinutes(
                setHours(new TZDate(occurrence, config?.timeZone), ensureDate(event.start, config?.timeZone).getHours()), // Use TZDate for occurrence
                ensureDate(event.start, config?.timeZone).getMinutes()
              );
        ...
        if (format(ensureDate(event.start, config?.timeZone), "yyyy-MM-dd HH:mm") === slotDateFormatted) {
          acc.push(event);
        }
        return acc;
      }, []);
          ...
          <div className="w-16 bg-gray-50 border-r border-gray-200">
            ...
            {timeSlots.map((index) => (
              <div key={`hour-${index}`} className="h-10 flex items-center justify-end pr-2 text-xs text-gray-500">
                {format(
                  setMinutes(
                    setHours(startOfDay(new TZDate(new Date(), config?.timeZone)), startHour + Math.floor((index * config?.slotDuration!) / 60)),
